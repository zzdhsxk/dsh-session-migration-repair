...
/**
 * Node gained zstd support in v22.15/v23.8. A namespace import keeps this module
 * loadable on older runtimes so the failure is a readable sentence instead of a
 * link-time "does not provide an export named" SyntaxError.
 */
function requireZstd() {
  if (typeof zlib.zstdDecompressSync !== "function" || typeof zlib.zstdCompressSync !== "function") {
    throw new Error(
      "dsh-session-migration-repair requires Node >= 22.15 (node:zlib zstd support); this runtime is " +
        process.version,
    );
  }
}
...
export function listFrames(buffer) {
  ...
  while (offset < buffer.length) {
    const start = offset;
    if (!isZstdFrame(buffer, offset)) throw new Error("not a zstd stream: invalid frame magic at byte " + offset + " (is this really a DSH session artifact?)");
    offset += 4;
...
export function encodeLog(lines, options = {}) {
  ...
  for (let index = 1; index < lines.length; index += frameLines) {
    ...
    frames.push(zlib.zstdCompressSync(Buffer.from(chunk, "utf8"), { level }));
  }
